...
#import "ZHFRealmManager.h"
#import "ZHFPruduct.h"
#import "ZHFPerson.h"

@implementation ZHFRealmManager
...
+ (RLMRealmConfiguration *)realmConfig {
    ...
    config.objectClasses = @[ZHFPruduct.class,ZHFPerson.class];
    config.readOnly = NO;
    ...
    config.migrationBlock = ^(RLMMigration *migration , uint64_t oldSchemaVersion) {
        // enumerateObjects:block: 遍历了存储在 Realm 文件中的每一个“ZHFPerson”对象
        [migration enumerateObjects:ZHFPerson.className block:^(RLMObject *oldObject, RLMObject *newObject) {
            ...
            if (oldSchemaVersion < 3) { // 重命名操作应该在调用 `enumerateObjects:` 之外完成
                [migration renamePropertyForClass:ZHFPerson.className oldName:@"yearsSinceBirth" newName:@"age"];
            }
        }];
    };
    return config;
